...
public class GLRendererex implements Renderer {
	...
	public GLRendererex(){ 
		cube = new GLCube((float)0.0); 
		cube2 = new GLCube((float)1.0); 
		cube3 = new GLCube((float)0.9); 
		cube4 = new GLCube((float)0.0); 
	}
	@Override
	...
	x :for(int i=0;i<180;i++) 
		...
		float angle1 = 0.07f * ((int)time1); 
		gl.glRotatef(angle1,0,0,10);
		gl.glPushMatrix();
		gl.glColor4f(1f,0f,0f,0.5f);
		cube2.draw(gl);
		gl.glPopMatrix();
...
//				gl.glTranslatef(4,0,0);
			...
			float angle3 = 0.0000000009f * ((int)time3); 
